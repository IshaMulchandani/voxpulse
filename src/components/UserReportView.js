...
const UserReportView = () => {
    ...
    const [votesRecords, setVotesRecords] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch report from publicReports collection
                ...
                if (auth.currentUser && !reportData.readBy?.includes(auth.currentUser.uid)) {
                ...
                <div className="report-content">
                    ...
                    {report?.type === 'poll' && poll ? (
                        ...
                        <div className="system-report-content">
                            <div className="system-report-placeholder">
                                <h3>📊 System Report</h3>
                                <p>This is a system report with administrative insights and analytics.</p>
                                <div className="report-metadata">
                                    <p><strong>Report Type:</strong> {report?.systemType || 'System Analysis'}</p>
                                    <p><strong>Generated:</strong> {new Date(report?.createdAt).toLocaleDateString()}</p>
                                    <p><strong>Views:</strong> {report?.views || 0}</p>
                                </div>
                            </div>
                        </div>
                    )}
